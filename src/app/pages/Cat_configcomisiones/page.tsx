...
const commonProps = {
  ...
  sx: {
    ...
    '& .MuiInputLabel-root': { transform: 'translate(14px, 12px) scale(1)', color: '#666', fontWeight: 500 },
...
export default function ConfigComisiones() {
  ...
  const handleGuardar = async () => {
      ...
      setSaving(true);
      try {
          const payload = { ...formData, comision: comisionVal, puesto: Number(formData.puesto) };
          ...
          if (res.status === 200) {
              ...
              fetchTablaComisiones();
              // Limpiamos montos y producto para rápida captura
    ...
    <Box sx={{ p: 2, minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      <Paper sx={{ p: 3, mb: 3, borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
        ...
        <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={3}>
                <TextField {...selectProps} select label="Puesto (Obligatorio)*" name="puesto" value={formData.puesto} onChange={handleInputChange} sx={{...selectProps.sx, '& .MuiOutlinedInput-notchedOutline': { borderColor: '#1976d2' }}}>
            ...
            <Grid item xs={12} md={4} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#f9f9f9', borderRadius: 2, border: '1px solid #eee' }}>
                <FormGroup row>
                    <FormControlLabel control={<Checkbox checked={formData.basePrecioConIva} onChange={handleCheckboxChange} name="basePrecioConIva" color="primary" />} label="Con IVA" />
                    <FormControlLabel control={<Checkbox checked={formData.basePrecioSinIIva} onChange={handleCheckboxChange} name="basePrecioSinIIva" color="secondary" />} label="Sin IVA" />
                </FormGroup>
            </Grid>
            <Grid item xs={12} md={2}>
            ...
            {/* --- SECCIÓN 4: DÍAS DE APLICACIÓN --- */}
            <Grid item xs={12}>
                <Box sx={{ p: 1, border: '1px solid #e0e0e0', borderRadius: '8px', bgcolor: '#fafafa' }}>
                    <Typography variant="caption" sx={{ fontWeight: 'bold', color: '#666', ml: 1 }}>DÍAS DE APLICACIÓN:</Typography>
                    <FormGroup row sx={{ justifyContent: 'space-around', mt: 0.5 }}>
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaL} onChange={handleCheckboxChange} name="aplicaL" />} label="Lunes" />
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaM} onChange={handleCheckboxChange} name="aplicaM" />} label="Martes" />
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaMi} onChange={handleCheckboxChange} name="aplicaMi" />} label="Miércoles" />
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaJ} onChange={handleCheckboxChange} name="aplicaJ" />} label="Jueves" />
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaV} onChange={handleCheckboxChange} name="aplicaV" />} label="Viernes" />
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaS} onChange={handleCheckboxChange} name="aplicaS" />} label="Sábado" />
                        <FormControlLabel control={<Checkbox size="small" checked={formData.aplicaD} onChange={handleCheckboxChange} name="aplicaD" />} label="Domingo" />
                    </FormGroup>
                </Box>
            </Grid>
        </Grid>
      ...
      {/* TABLA PRINCIPAL */}
      <TableContainer component={Paper} sx={{ maxHeight: 500, borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.08)' }}>
        <DataGrid 
            ...
            sx={{ border: 'none', '& .MuiDataGrid-columnHeaders': { backgroundColor: '#f0f0f0', fontWeight: 'bold' } }} 
        />
      </TableContainer>
